...
async function sticker(img, url, ...args) {
  ...
  let effectOptions = ['-c', '-p', '-x']
  if (effectOptions.includes(option)) {
    args.pop()
    try {
      if (option === '-c') img = await makeCircular(img)
      else if (option === '-p') img = await resizeSmall(img)
      else if (option === '-x') img = await applyEffect(img)
    } catch (e) {
