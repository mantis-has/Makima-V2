...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  try {
    ...
    if (/webp|image|video/g.test(mime)) {
      if (/video/g.test(mime)) 
        if ((q.msg || q).seconds > 8) 
          return m.reply(`${emoji} *¡El video no puede durar mas de 8 segundos!*`)
    ...
    if (stiker) {
      ...
      return conn.reply(m.chat, `${emoji} La conversión ha fallado, intenta enviar primero imagen/video/gif y luego responde con el comando.`, m, rcanal)
    }
  }
}
